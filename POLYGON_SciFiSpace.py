...
def import_characters():
    ...
    folder = bpy.path.abspath("//_SourceFiles\\Characters")
    fbxs = [f for f in os.listdir(folder) if \
        f.endswith(".fbx") \
        and not "_Suit_" in f \
        and not "_Head_" in f \
        and not f.startswith("SK_Chr_Attach_") \
        # The SpaceSoldier's not ending in 01.fbx are ASCII. Skip them
        and not (f.startswith("SK_Chr_SpaceSoldier_") and not f.endswith("01.fbx"))
    ]
    ...
    for obj in collection.objects:
        ...
        if first_armature != None and obj.type == 'MESH':
            ...
            texture_path = bpy.path.abspath("//_SourceFiles\\Textures\\PolygonSciFiSpace_Texture_01_A.png")
    ...
    for image in bpy.data.images.values():
        if image.source == "FILE":
            ...
            image.filepath = bpy.path.abspath(
                "//_SourceFiles\\Textures\\" + filename
            )
