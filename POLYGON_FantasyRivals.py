...
def import_characters():
    ...
    fbxs = [f for f in os.listdir(folder) if f.endswith(".fbx") and (f.startswith("SK_BR_Char") or f.startswith("SK_Char"))]
    ...
    for image in bpy.data.images.values():
        ...
        if image.source == "FILE":
            if filename == "Texture_A1.png" or filename == "FantasyRivals_Justin_Main_01.psd":
                # Make absolute
                image.filepath = bpy.path.abspath(
                    "//Source_Files\\Textures\\FantasyRivals_Texture_01_A.png"
                )
            else:
                # Make absolute
                image.filepath = bpy.path.abspath(
